...
export default function App() {
    ...
    try {
      ...
      if (res.ok && json.status === 1) {
        if (target === 'grades') {
          ...
          setTimetableSem(json.semester)
        } else {
          setSchedule(json.data)
          setScheduleSem(json.semester)
        }
      } else {
  ...
  useEffect(() => {
    if (auth && grades.length === 0) sync('grades'), sync('timetable')
  }, [auth])
  const getSubtitle = () => {
    if (tab === 'grades') return 'ACADEMIC TRANSCRIPT'
    if (tab === 'schedule') return scheduleSem ? formatSemester(scheduleSem) : 'WAITING FOR SYNC...'
    if (tab === 'timetable') return timetableSem ? formatSemester(timetableSem) : 'WAITING FOR SYNC...'
    return ''
  }
  const getSyncTarget = () => {
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-300 p-8 font-sans selection:bg-blue-500/30">
        ...
        <div className="flex gap-4 items-center">
          <div className="flex bg-zinc-900 p-1.5 rounded-2xl border border-zinc-800">
            <button
              ...
              className={`px-8 py-3 rounded-xl text-xs font-black transition-all ${tab === 'timetable' ? 'bg-zinc-950 text-white shadow-lg ring-1 ring-zinc-800' : 'text-zinc-500 hover:text-zinc-300'}`}
            >
              课表
            </button>
            <button
      ...
      <main className="max-w-[100rem] mx-auto pb-20 animate-fade-in-up">
        ...
        {tab === 'schedule' && <ScheduleView data={schedule} />}
        {tab === 'timetable' && <TimetableView data={timetable} />}
      </main>
    </div>
  )
}